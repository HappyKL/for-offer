...
- 根据算法调度
  ...
  - DH： 目标地址散列，同一个目标地址的用户请求发送给同一个真实服务器，可以提高缓存命中率

### 动态算法

- 除了根据算法调度，也考虑服务器状态
  - LC 最少连接：分配给连接数最少的  活动连接*256+非活动连接
  ...
  - LBLC 特殊的DH算法：基本采用DH算法，但当服务器压力较大时，添加其他服务器负载。既能提高缓存命中率，又要考虑服务器性能。比如：微博某个热点被挤爆，这种算法可以添加负载均衡
...
- 持久端口连接 PPC：将来自同一个客户端同一个服务端口的请求，始终定向至此前选定的RS
...
- 持久防火墙标记连接：将来自同一客户端对指定服务端口的请求，始终定向至此选定的RS
